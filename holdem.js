var holdem = {
	...
	card5: 9,
	card6: 25,
	monteCarloMaxIteration: 20000,
	
	/**
	 * Initialize hand cards and pushed into an array
	 */
	handValue: function(card0, card1, card2, card3, card4, card5, card6) {
		this.card0 = card0;
		this.card.push(card0);
		this.card1 = card1;
		this.card.push(card1);
		this.card2 = card2;
		this.card.push(card2);
		this.card3 = card3;
		this.card.push(card3);
		this.card4 = card4;
		this.card.push(card4);
		this.card5 = card5;
		this.card.push(card5);
		this.card6 = card6;
		this.card.push(card6);
	 ...
	 * HandValue array
	 */
	handArray: function(cards){
		//console.log(cards);
		this.card0 = cards[0];
		this.card.push(cards[0]);
		this.card1 = cards[1];
		this.card.push(cards[1]);
		this.card2 = cards[2];
		this.card.push(cards[2]);
		this.card3 = cards[3];
		this.card.push(cards[3]);
		this.card4 = cards[4];
		this.card.push(cards[4]);
		this.card5 = cards[5];
		this.card.push(cards[5]);
		this.card6 = cards[6];
		this.card.push(cards[6]);
	  ...
	  Output :
		...
		- 4 leading bits representing the category of the best 5 card poker
		  hand that can be made out of the 7 cards :
			...
			1 (0x1) => one pair,						一对
			2 (0x2) => two pairs,						两对
			3 (0x3) => three of a kind a.k.a. trip,		三条
			4 (0x4) => straight,						顺子
			5 (0x5) => flush,							同花
			6 (0x6) => full house,						满堂红
			7 (0x7) => four of a kind a.k.a. quad,		四条
			8 (0x8) => straight flush. 					同花顺
		 ...
		 of aces.
	*/
	naiveHandEval: function(card0, card1, card2, card3, card4, card5, card6) {
		var cardArr = Array();
		var length;
		cardArr.push(card0);
		cardArr.push(card1);
		...
		cardArr.push(card6);
		//console.log(cardArr);
		length = cardArr.length;
		
		var handValue;
		function completeHandValue(cards) {
			/* A local function to add the top cards of a given array to a partially calculated handValue. */
			if (handValue < 0x10) { 
				// put in a sentinel: when that bit
	...
	/**
	 * Mane Random Hand Value different current hand value
	 */
	makeRandomHand: function(bannedArray) {
		if(bannedArray == undefined)
			return ;
		...
		for(var i = 0; i < this.count; i++){
			var generatedNumber;
			for (;;){
				generatedNumber = this.getRandomInt(0, 51);
				exist = false;
		...
		//console.log(generatedHand);
		return generatedHand;
	},
	
	/**
	 * Generate random int value from limit value
	 */
	 ...
	 *	Enumeration ?
	 */
	exhaustive: function(playsNumber){
		...
		for (hand[0] = 0; hand[0] < 51; hand[0]++) {
			for (hand[1] = hand[0]+1; hand[1] < 52; hand[1]++) {
				...
				if (hand[0] != p1[0] && hand[0] != p1[1] && hand[0] != alreadyboard[0] && hand[0] != alreadyboard[1] && hand[0] != alreadyboard[2] 
					&& hand[0] != alreadyboard[3] && hand[0] != alreadyboard[4]  && hand[1] != p1[0] && hand[1] != p1[1] && hand[1] != alreadyboard[0] 
					&& hand[1] != alreadyboard[1] && hand[1] != alreadyboard[2] && hand[1] != alreadyboard[3] && hand[1] != alreadyboard[4] ){
		...
		var rt_obj = {
			...
			'winOdds': winOdds,
			'tieOdds': tieOdds,
			'slaveOdds': (1.0-tieOdds-winOdds),
			'cardCounts': globalCount,
		}
/*
...
*/
	 ...
	 */
	monteCarloSimulation: function(playersNumber, phase) {
		var oppCount = parseInt(playersNumber) - 1;
		var alreadyBoardCount = 0;
		var boardCount = 5;

		var ph = parseInt(phase);
		if (ph == 0)
			phase = 'pre-flop';
		else if (ph == 1)
			phase  = 'flop';
...
/*
holdem.handValue(0,20,1,4,5,9,25);
console.log(holdem.naiveHandEval(0,20,1,4,5,9,25));
holdem.exhaustive(2);
holdem.monteCarloSimulation(2,0);
*/